        ...
        return {
            ...
            paint: function ( $element, layout ) {
                ...
                if(document.getElementById(id)) {
                    ...
                    $element.append($('<div style="overflow-y:auto;overflow-x:hidden;">').attr("id", id).width(width).height(height-150));
                ...
                drawChart(svg, data, options, width, height,dimLabels,measureLabels);
            }
...
function drawChart(svg, data, options, w, h,dl,ml) {
      ...
      var row = 0,
          ...
          left = (options.chartOrientation == "horizontal" ? 200 : (options.chartType == "pie" ? 25 : 50)),
          inMarg = 50;
      

      var width = ((w-left) / Math.min(tDim.length, options.nCols)) - inMarg;
      var height = 100;//((h-((options.chartType == "pie" ? 40 : 120)-inMarg)) / Math.min(tDim.length, options.nRows)) - inMarg ;
      var totalWidth = w;
      ...
      tDim.forEach(function (tDimValue) {
          ...
          if (options.trellisLabelPos == "inside") {
            ...
            svg
            .append("text")
                .attr("x", left + (col * (width + inMarg)) + width/2 )// (width + inMarg)
                .attr("y", top + (row * (1.4*height+ inMarg)) - Math.min(10, options.trellisLabelSize) )
          ...
          if (options.chartType !== "pie") {
             ...
             /* if (col > 0 || options.measureLabelHide) {
              ...
              //else {
                ...
                x.shapes.select("path.dimple-custom-axis-line").style("stroke", "#E1E1E1");
                x.shapes.selectAll("text").style("fill", "#979797");
                y.shapes.select("path.dimple-custom-axis-line").style("stroke", "#E1E1E1");
                y.shapes.selectAll("text").style("fill", "#979797");
             // }
